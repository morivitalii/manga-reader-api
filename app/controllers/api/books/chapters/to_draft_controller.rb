...
class Api::Books::Chapters::ToDraftController < Api::ApplicationController
  before_action :set_book, only: [:update]
  before_action :set_chapter, only: [:update]

  before_action -> { authorize(Api::Books::Chapters::ToDraftPolicy, chapter: @chapter) }, only: [:update]

  def update
    service = Api::Books::Chapters::ToDraft.new(chapter: @chapter)

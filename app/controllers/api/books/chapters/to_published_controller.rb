class Api::Books::Chapters::ToPublishedController < Api::ApplicationController
  before_action :set_book, only: [:update]
  before_action :set_chapter, only: [:update]

  before_action -> { authorize(Api::Books::Chapters::ToPublishedPolicy, chapter: @chapter) }, only: [:update]

  def update
    service = Api::Books::Chapters::ToPublished.new(chapter: @chapter)

class Api::Books::ToDraftController < Api::ApplicationController
  before_action :set_book, only: [:update]

  before_action -> { authorize(Api::Books::ToDraftPolicy, book: @book) }, only: [:update]

  def update
    service = Api::Books::ToDraft.new(book: @book)

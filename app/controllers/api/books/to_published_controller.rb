class Api::Books::ToPublishedController < Api::ApplicationController
  before_action :set_book, only: [:update]

  before_action -> { authorize(Api::Books::ToPublishedPolicy, book: @book) }, only: [:update]

  def update
    service = Api::Books::ToPublished.new(book: @book)
